...
//
// FALLBACK: If the database is unavailable, pricing falls back to hardcoded
// values in /server/config/hardcodedPricing.js. This ensures quotes can
// always be generated, even if SQLite is temporarily down.

const db = require('./db');
const hardcoded = require('./config/hardcodedPricing');
...
function buildLivePricingTables() {
  ...
  let tiers, locations, locationPricing;
  
  try {
    tiers = db.prepare('SELECT quantity, standard_price, hard_floor_price FROM pricing_tiers ORDER BY quantity').all();
    locations = db.prepare('SELECT * FROM print_locations WHERE active = 1 ORDER BY sort_order').all();
    locationPricing = {};
    ...
    console.warn('[FALLBACK] Database query failed, using hardcoded pricing:', dbError.message);
    useHardcoded = true;
    
    tiers = hardcoded.HARDCODED_PRICING_TIERS.map(([qty, std, floor]) => ({
      quantity: qty, standard_price: std, hard_floor_price: floor
    }));
    
    locations = hardcoded.HARDCODED_PRINT_LOCATIONS.map(loc => ({
      id: loc.id, name: loc.name, code: loc.code, 
      included_in_base: loc.includedInBase, 
      internal_cost_per_unit: loc.internalCostPerUnit,
      sort_order: loc.sortOrder, active: 1
    }));
    
    locationPricing = {};
    for (const loc of hardcoded.HARDCODED_PRINT_LOCATIONS) {
      locationPricing[loc.id] = Object.fromEntries(loc.pricingMatrix);
    }
  }
  ...
  return {
    ...
    locationPricing,
    costs: {
      blank_cost: useHardcoded ? hardcoded.HARDCODED_COST_SETTINGS.blank_cost : getSettingNum('blank_cost', 3.5),
      front_transfer_cost: useHardcoded ? hardcoded.HARDCODED_COST_SETTINGS.front_transfer_cost : getSettingNum('front_transfer_cost', 2.75),
      labor_cost: useHardcoded ? hardcoded.HARDCODED_COST_SETTINGS.labor_cost : getSettingNum('labor_cost', 2.5),
      back_transfer_cost: useHardcoded ? hardcoded.HARDCODED_COST_SETTINGS.back_transfer_cost : getSettingNum('back_transfer_cost', 2.75),
    },
    designSizeSurcharges: {
      standard: 0,
      large: useHardcoded ? hardcoded.HARDCODED_COST_SETTINGS.design_size_large_surcharge : getSettingNum('design_size_large_surcharge', 1.50),
      oversized: useHardcoded ? hardcoded.HARDCODED_COST_SETTINGS.design_size_oversized_surcharge : getSettingNum('design_size_oversized_surcharge', 2.50),
    },
    usedHardcodedFallback: useHardcoded,
  };
}
...
function calculateQuote(input, pricingTables) {
  ...
  // 250+ requires a custom quote / review by admin
  if (totalQty >= 250) {
